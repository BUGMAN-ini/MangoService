...
namespace Mango.Services.ShoppingCart.API.Controllers
{
    ...
    public class ShoppingCartAPIController : ControllerBase
    {
        ...
        public ShoppingCartAPIController(IMapper mapper, AppDbContext db, IProductService productService, IMessageBus messageBus, IConfiguration configuration)
        {
            _mapper = mapper;
            _db = db;
        ...
        public async Task<object> EmailCartRequest([FromBody] CartDto cartdto)
        {
            try
            {
                await messageBus.PublishMessage(cartdto, _configuration.GetValue<string>("TopicAndQueueNames:EmailShoppingCartQueue"));
                _response.Result = true;
            }
            catch (Exception ex)
            {
